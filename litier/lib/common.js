...
$(document).on(change, .selectAll, function() {  addremove all characters
    if($(this).is(checked)) {
...
function alignPopup(popup) {
 ...
 httpsstackoverflow.coma3971432 thanks Zack Katz D
...
function loadFilterList(allTags) {
    ...
    allTags.forEach(function(tag, i) {
        $(#filterChecklist divlast-child).after(filterCheckWrap);
        $(#filterChecklist divlast-child).html(tag +  input type='checkbox' class='filterCheck' id='+tag+' );
    });
}
